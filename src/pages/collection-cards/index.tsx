...
import { useRoqClient, useCollectionCardFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<CollectionCardInterface, unknown>;

interface CollectionCardListPageProps {
...
export function CollectionCardListPage(props: CollectionCardListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['condition.contains'],
          ...params.filters,
        },
        'collection_card',
      ),
  ...
  const { data, error, isLoading, mutate } = useCollectionCardFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.collection_card.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: CollectionCardInterface) => {
    if (hasAccess('collection_card', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/collection-cards/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) => (record?.added_at ? format(record?.added_at, 'dd-MM-yyyy') : ''),
    },
    { id: 'purchase_price', header: 'purchase_price', accessorKey: 'purchase_price' },
    { id: 'current_value', header: 'current_value', accessorKey: 'current_value' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link
              ...
              href={`/collections/view/${record.collection?.id}`}
            >
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/collection-cards/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="collection_card" />
        </Flex>
