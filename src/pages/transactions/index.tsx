...
export function TransactionListPage(props: TransactionListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'game_card'],
          searchTerm: params.searchTerm,
          searchTermKeys: [
            'transaction_type.contains',
            'payment_method.contains',
            'billing_address.contains',
            'shipping_address.contains',
            'status.contains',
          ],
          ...params.filters,
        },
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'billing_address', header: 'Billing Address', accessorKey: 'billing_address' },
    { id: 'shipping_address', header: 'Shipping Address', accessorKey: 'shipping_address' },
    ...
    hasAccess('game_card', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'game_card',
          header: 'Game Card',
          accessorKey: 'game_card',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/game-cards/view/${record.game_card?.id}`}>
              {record.game_card?.name}
            </Link>
          ),
        }
      : null,
