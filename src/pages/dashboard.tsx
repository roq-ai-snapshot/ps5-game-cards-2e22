...
import { useRoqPlatformUserProfiles, useGameCardCount, useCollectionCount, useCollectionCardCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const gameCardStat = useGameCardCount();

  const collectionStat = useCollectionCount();

  const collectionCardStat = useCollectionCardCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Game Cards',
      result: gameCardStat?.data,
      isLoading: gameCardStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Collections',
      result: collectionStat?.data,
      isLoading: collectionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Collection Cards',
      result: collectionCardStat?.data,
      isLoading: collectionCardStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
