...
import { useRoqClient, useGameCardFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<GameCardInterface, unknown>;

interface GameCardListPageProps {
...
export function GameCardListPage(props: GameCardListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'company', 'collection_card.count', 'transaction.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['name.contains', 'description.contains', 'genre.contains'],
          ...params.filters,
        },
        'game_card',
      ),
  ...
  const { data, error, isLoading, mutate } = useGameCardFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.game_card.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: GameCardInterface) => {
    if (hasAccess('game_card', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/game-cards/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('collection_card', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'collection_card',
          cell: ({ row: { original: record } }: any) => record?._count?.collection_card,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/game-cards/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('game_card', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/game-cards/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('game_card', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="game_card" />
        </Flex>

        {hasAccess('game_card', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/game-cards/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
