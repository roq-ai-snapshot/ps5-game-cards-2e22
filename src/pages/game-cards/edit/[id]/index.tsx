...
import { useRoqClient, useGameCardFindFirst } from 'lib/roq';
...
function GameCardEditPage() {
  ...
  const { data, error, isLoading, mutate } = useGameCardFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: GameCardInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.game_card.update({
        data: values as RoqTypes.game_card,
        where: {
          id,
        },
      });
