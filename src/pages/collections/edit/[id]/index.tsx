...
import { useRoqClient, useCollectionFindFirst } from 'lib/roq';
...
function CollectionEditPage() {
  ...
  const { data, error, isLoading, mutate } = useCollectionFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: CollectionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.collection.update({
        data: values as RoqTypes.collection,
        where: {
          id,
        },
      });
